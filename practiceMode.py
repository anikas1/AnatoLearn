...
Description: Runs the first level of the Practice Mode, where the wisdom teeth must be properly removed from their place.
"""

#import statements to fetch modules
import os, pygame
from Drag import *
from wrongTeethCoordinates import *
import time
...
pygame.display.set_caption('Practice Mode - Wisdom Teeth Removal') #sets the window caption

#Body Img Source: https://www.mein-stoffwechsel.com/
#Background Img Source: https://www.istockphoto.com/photo/abstract-interior-design-of-modern-showroom-with-empty-concrete-floor-and-white-wall-gm917163050-252333626
background = pygame.image.load("images\practicemode1bg.png") #uploads the practice mode background
screen.blit(background, (0,0)) #projects the background at (0,0)
teeth = pygame.image.load("images/teethImg.png")
...
i1 = load_image("C:/Anika/School/ECG/11th Grade/3 wk - MATH 241 - Scientific Computing/AnatoLearn/images/tl.png", 0, 0)
i2 = load_image("C:/Anika/School/ECG/11th Grade/3 wk - MATH 241 - Scientific Computing/AnatoLearn/images/tr.png", 0, 0)
i3 = load_image("C:/Anika/School/ECG/11th Grade/3 wk - MATH 241 - Scientific Computing/AnatoLearn/images/bl.png", 0, 0)
i4 = load_image("C:/Anika/School/ECG/11th Grade/3 wk - MATH 241 - Scientific Computing/AnatoLearn/images/br.png", 0, 0)

#calling the Drag class and passing arguments to its constructor
topleft = Drag(i1, (0, 0), 405, 129)
topright = Drag(i2, (0, 0), 564, 129)
bottomleft = Drag(i3, (0, 0), 407, 228)
bottomright = Drag(i4, (0, 0), 564, 230)


#uses the pygame.sprite method RenderPlain to render the images
allsprites = pygame.sprite.RenderPlain((topleft, topright, bottomleft, bottomright))
    ...
    for event in pygame.event.get():
        ...
        #checks if mouse button is down
            ...
            if(x > 36) and (x < 75):
                if (y > 469) and (y < 505):
                    bgMusic.stop() 
                    exec(open("PMLevel1Intro.py").read()) #opens up homescreen when back button is clicked
            if (x > 89) and (x < 122):
                if (y > 470) and (y < 498):
                    bgMusic.stop() 
            ...
            topleft.rollover() 
            x1 = topleft.rect.x #x pos of tooth
            y1 = topleft.rect.y #y pos of tooth
            #if the tooth is dragged inside one of the boxes, add 200 points, and the counter is still 0
            if(((x1 > 152) and (x1 < 240)) or ((x1 > 718) and (x1 < 807))) and tl == 0:
            ...
            x2 = topright.rect.x
            y2 = topright.rect.y
            if(((x2 > 152) and (x2 < 240)) or ((x2 > 718) and (x2 < 807))) and tr == 0:
                if (y2 > 108) and (y2 < 284):
                    if(x > 442) and (x < 518):
                        if (y > 498) and (y < 519):
                            score += 200
                            bgMusic.stop()
            ...
            bottomleft.rollover()
            x3 = bottomleft.rect.x
            y3 = bottomleft.rect.y
            if(((x3 > 152) and (x3 < 240)) or ((x3 > 718) and (x3 < 807))) and bl == 0:
                if (y3 > 108) and (y3 < 284):
                    if(x > 442) and (x < 518):
                        if (y > 498) and (y < 519):
                            score += 200
                            bgMusic.stop()
            ...
            bottomright.rollover()                                     
            x4 = bottomright.rect.x
            y4 = bottomright.rect.y
            if(((x4 > 152) and (x4 < 240)) or ((x4 > 718) and (x4 < 807))) and br == 0:
                if (y4 > 108) and (y4 < 284):
                    if(x > 442) and (x < 518):
                        if (y > 498) and (y < 519):
                            score += 200
                            bgMusic.stop()
        ...
        if tl == 1 and tr == 1 and bl == 1 and br == 1:
            bgMusic.stop() #stops music
            time.sleep(3) #wait 3 seconds to see score
            exec(open("pMLevel1Comp.py").read()) #launches next screen
